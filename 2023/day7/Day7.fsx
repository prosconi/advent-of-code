...
let go lines =
    lines
    |> parse
    |> Seq.map (fun (cards, bid) -> 
        let handRank = getHandRank cards
        let handRanking = getHandRanking cards
        cards, handRank, handRanking, bid
    )
    |> Seq.sortBy (fun (_, handRank, handRanking, _) -> handRank, handRanking)
    |> Seq.mapi (fun i (_cards, _handRank, _handRanking, bid) -> bid, i + 1)
    |> Seq.map (fun (x, y) -> x * y)
