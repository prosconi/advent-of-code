...
|> Seq.iter (fun x -> 
    if x.Contains "=" then
        let left, right = splitIntoTwo "=" x
        let index = hash left
        let value = left, int right

        match boxes[index].FindIndex(fun (l, _) -> l = left) with
        | -1 -> boxes[index].Add(value)
        | i -> boxes[index].[i] <- value
    else
        let left, _ = splitIntoTwo "-" x
        let index = hash left
        boxes[index].RemoveAll (fun (l, _) -> l = left) |> ignore
)
