...
let swap (data: ResizeArray<_>) (i1, i2) =
    let temp = data[i1]
    data[i1] <- data[i2]
    data[i2] <- temp
...
let calc (data: string) =
    let asArray =
        ...
        |> Seq.indexed
        |> Seq.collect (fun (i, (usedSpace, freeSpace)) -> 
            [|
    ...
    let asResizeArray = new ResizeArray<string>(asArray)
    while not <| isCompact asResizeArray do
        let firstDot = asResizeArray |> Seq.findIndex (fun c -> c = ".")
        let lastDot = asResizeArray |> Seq.findIndexBack (fun c -> c <> ".")
        swap asResizeArray (firstDot, lastDot)
    checksum asResizeArray
