...
let toValues dimensions data =
    data
    |> Array.filter (isSafe dimensions)
    |> Array.map _.Position
    |> Array.groupBy id
    |> Array.map (fun (k, v) -> (k, Array.length v))

let toQuadrants (width, height) data =
    data
    |> toValues (width, height)
    |> Array.map (fun (k, v) -> 
        let x, y = k
        if x < width / 2 && y < height / 2 then (1, v)
        else if x >= width / 2 && y < height / 2 then (2, v)
        else if x < width / 2 && y >= height / 2 then (3, v)
        else if x >= width / 2 && y >= height / 2 then (4, v)
        else failwith "Error in code"
    )
...
let printRobots (width, height) data =
    ...
    for coords, v in toValues (width, height) data do
        let x, y = coords
...
let simulate() =
    ...
    d
    |> toQuadrants (width, height)
    |> Seq.groupBy fst
    |> Seq.map (fun (q, v) -> v |> Seq.map snd |> Seq.sum)
    |> Seq.fold (*) 1
